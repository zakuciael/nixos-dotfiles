      ...
      plugins =
        let
          ...
          selectFile =
            id: ide: build:
            let
              pluginName = pluginsJson.plugins."${id}".name;
            in
            if !builtins.elem ide pluginsJson.plugins."${id}".compatible then
              throw "Plugin ${pluginName} (${id}) is does not support ${ide} IDE."
            else if !pluginsJson.plugins."${id}".builds ? "${build}" then
              throw "IDE ${ide} (${build}) was not found in ${pluginName} (${id}) plugin info. Try ./update_plugins.py --with-build"
            else if pluginsJson.plugins."${id}".builds."${build}" == null then
              throw "Plugin ${pluginName} (${id}) is not compatible with ${ide} (${build}) IDE."
            else
        ...
        in
        {
          ...
          addPlugins =
            ...
            let
              processPlugin =
                ...
                if isDerivation plugin then
                  ...
                  byId."${plugin}" (ide.baseName or ide.pname) ide.buildNumber
                else if byName ? "${plugin}" then
                  byName."${plugin}" (ide.baseName or ide.pname) ide.buildNumber
                else
            ...
            stdenv.mkDerivation rec {
              ...
              nativeBuildInputs =
                (optional stdenv.hostPlatform.isLinux autoPatchelfHook) ++ (ide.nativeBuildInputs or [ ]);
